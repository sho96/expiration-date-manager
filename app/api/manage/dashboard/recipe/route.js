...
import { StreamingResponse } from "@/utils/server/streaming";
...
function formattedItemsToString(items, language="en") {
  return items.map((item) => `${termTable["name"][language]}: ${item.name}, ${termTable["category"][language]}: ${item.category}, ${termTable["expiration date"][language]}: ${item.expirationDate}`).join("\n");
}

function formattedLeftoversToString(leftovers, language="en") {
  return leftovers.map((leftover) => `${termTable["name"][language]}: ${leftover.name}, ${termTable["expiration date"][language]}: ${leftover.expirationDate}`).join("\n");
}
