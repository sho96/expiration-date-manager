...
function formattedItemsToString(items, language = "en") {
  ...
  return items
    .map(
      (item) =>
        `${termTable["name"][language]}: ${item.name}, ${
          termTable["category"][language]
        }: ${item.category}, ${
          termTable["expiration date"][language]
...
function formattedLeftoversToString(leftovers, language = "en") {
  ...
  return leftovers
    .map(
      (leftover) =>
        `${termTable["name"][language]}: ${leftover.name}, ${
          termTable["expiration date"][language]
        }: ${leftover.expirationDate.toLocaleDateString()}`
    )
    .join("\n");
}
